...
def babel_convert(fname1, fmt1, fname2, fmt2, verbose=0):
    ...
    cmd = "%s -i%s %s -o%s %s 2> /dev/null" % (
        BABEL_PATH, fmt1, fname1, fmt2, fname2)
    if verbose:
        sys.stderr.write('\n\n\n\nBABEL VERBOSE:')
        sys.stderr.write("%s\n" % cmd)
    status = os.system(cmd)

def babel_convert_split(fname1, fmt1, fname2, fmt2, verbose=0):
    cmd = "%s -i%s %s -m -o%s %s 2> /dev/null" % (
        BABEL_PATH, fmt1, fname1, fmt2, fname2)
    if verbose:
        sys.stderr.write('\n\n\n\nBABEL VERBOSE:')
        sys.stderr.write("%s\n" % cmd)
...
def babel_reprotonate_mol2_old(filePrefix, oriExt=".ori.mol2", endExt=".mol2", verbose=False):
    ...
    """
    # Delete hydrogens
    cmd = "%s -d -imol2 %s -omol2 %s 2> /dev/null" % (
        BABEL_PATH, filePrefix + oriExt, filePrefix + ".babelskip.mol2")
    if not verbose:
        cmd = "%s 2> /dev/null" % cmd
    os.system(cmd)
    # Build hydrogens
    cmd = "%s -p -imol2 %s -omol2 %s 2> /dev/null" % (
        BABEL_PATH, filePrefix + ".babelskip.mol2", filePrefix + endExt)
    if not verbose:
        cmd = "%s 2> /dev/null" % cmd
    os.system(cmd)
...
def babel_reprotonate_mol2(filePrefix, oriExt=".ori.mol2", endExt=".mol2", verbose=False):
    ...
    NOTE: NOT COMPATIBLE WITH SEVERAL CALLS OF THIS IN SAME DIR ... SHOULD USE TEMPFILES
    """
    cmd = "%s -imol2 %s -omol2 %s 2> /dev/null" % (
        BABEL_PATH, filePrefix + oriExt, filePrefix + ".babelskip.mol2")
    if not verbose:
        cmd = "%s 2> /dev/null" % cmd
    os.system(cmd)
    cmd = "%s -h -imol2 %s -omol2 %s 2> /dev/null" % (
        BABEL_PATH, filePrefix + ".babelskip.mol2", filePrefix + endExt)
    if not verbose:
        cmd = "%s 2> /dev/null" % cmd
    os.system(cmd)
...
class iMolecule:
    ...
    def __init__(self, smiles=None, sdf=None, sdfMolecule=None, id=None, axialEquatorial=None, createFromSdf=False, extractCoordsFromSdf=False, sampleStereo=False, verbose=False):
        ...
        if smiles and not createFromSdf:
            ...
            try:
                if verbose:
                    sys.stderr.write(
                        "iMolecule: Attempting molecule instance from %s\n" % smiles)
            ...
            sdfFile.close()
            os.system(
                BABEL_PATH + " -ismi tmpSmiForSdf.smi -osdf tmpSdfFromSmi.sdf 2> /dev/null")
            ...
            try:
                ...
                # Do not affect dihedrals ...
                self.defaultDihedrals(setAngle=False)
        ...
        if not sdfMolecule:
            ...
            for i in range(self.numberOfNonHAtoms):
                ...
                if (hcount == 1) and (self.molecule.atoms[i].chiral_class != 0):
                    ...
                    if i == 0:
                        hPos = 0
                    self.molecule.atoms[i].oatoms.insert(
                        hPos, self.molecule.atoms[i].oatoms.pop())
                    self.molecule.atoms[i].bonds.insert(
                        hPos, self.molecule.atoms[i].bonds.pop())
            ...
            for i in range(self.numberOfBondsWithoutH):
                ...
                if self.molecule.bonds[i].bondtype == 2 or self.molecule.bonds[i].bondtype == 4:
                    ...
                    if len(bonds) == 2:
                        ...
                        self.molecule.bonds[i].dbo.append(
                            (bonds[0][0], bonds[1][0], stereo))
                        nBondsZE += 1
        """nSlash = 0
            ...
            for a in asa:
                ...
                if verbose:
                    sys.stderr.write("%s %d -> %d" %
                                     (a.symbol, a.index, a.chiral_class))
            if verbose:
                sys.stderr.write("\n")
        for at in self.molecule.atoms:
        ...
        if verbose:
            ...
            for bond in self.molecule.bonds:
                if bond.dbo:
                    sys.stderr.write("Bond %s-%s-%s %s-%s-%s %s : symbol %s dbo %s\n" % (str(bond.atoms[0].symbol), str(bond.atoms[0].index), str(bond.atoms[0].handle), str(
                        bond.atoms[1].symbol), str(bond.atoms[1].index), str(bond.atoms[1].handle), str(bond.bondtype), str(bond.symbol), str(bond.dbo)))
                else:
                    sys.stderr.write("Bond %s-%s-%s %s-%s-%s %s : symbol %s\n" % (str(bond.atoms[0].symbol), str(bond.atoms[0].index), str(
                        bond.atoms[0].handle), str(bond.atoms[1].symbol), str(bond.atoms[1].index), str(bond.atoms[1].handle), str(bond.bondtype), str(bond.symbol)))
    ...
    def detectFusedRings(self):
        ...
        # on parcours la liste des cycles de la molecule
        for i in range(len(self.molecule.cycles)):
            if i in index:  # si on a deja traite ce cycle on passe au suivant
                continue
            else:  # sinon
                # on recupere tout les atomes du cycle -> x
                x = self.molecule.cycles[i].atoms
                index.append(i)
                j = i + 1
                # on regarde les autres cycles de la molecule
                while j < len(self.molecule.cycles):
                    if j in index:
                        j += 1
                        continue
                    else:
                        # on verifie si les 2 cycles consideres on au moins 1 atome en commun.
                        fr = conc(x, self.molecule.cycles[j].atoms, 1)
    ...
    def bricks(self):
        ...
        # on recupere une liste des cycles fusionnes de la molecule
        ...
        for i in range(len(fr)):  # on parcours la liste des cycles fusionnes
            for atom in fr[i]:  # pr chacun des atomes de ces cycles
                for v in atom.oatoms:  # on regarde leurs voisins
                    # si un des voisins n'est pas ds le cycle que l'on considere
                    if v not in fr[i]:
                        # ca veut dire que l'atome du cycle que l'on considere est lie a un linker/appendice, si ca n'est pas un H ...
                        if atom.handle not in bond:
                            for oatom in atom.oatoms:
                                if (oatom not in fr[i]) and not (oatom.symbol == 'H'):
                                    # on recupere son 'handle' (de l'atome)
                                    bond.append(atom.handle)
                                    break
                            # bond.append(atom.handle) # on recupere son 'handle' (de l'atome)
        # print fr[i]
            # on isole le supercycle du reste de la molecule
            brique.molecule.pruneToAtoms(fr[i])
            ...
            while z < len(brique.molecule.cycles):
                for atom in brique.molecule.cycles[z].atoms:
                    if atom.index in index:
                        brique.molecule.cycles.remove(
                            brique.molecule.cycles[z])
                        index = []
                        break
                    else:
            ...
            # creation d'une instance de Brique -> ajoute a cycles
            cycles.append(Brique(brique.molecule, bond))
        ...
        # composants connectes de la molecule sous la forme [(atoms, bonds), (atoms, bonds)] -> la
        la = Components.components(linker.molecule)
        ...
        for j in range(len(la)):
            la2.append(la[j][0])
        # creation d'une liste d'indice des atomes des linkers et appendices
        listIndex = sortIndex(la2)
        for i in range(len(la)):
            for index in listIndex[i]:
                # pr chaque tuple, on recupere la liste des atomes -> toIsolate
                toIsolate.append(linker.molecule.atoms[index])
            # on isole ces atomes de la molecule
            linker.molecule.pruneToAtoms(toIsolate)
            # le fragment isole n'est pas un cycle donc il faut enlever les cycles de l'attribut 'cycles'
            linker.molecule.cycles = []
            nbH = 0
            for atom in linker.molecule.atoms:
                if atom.symbol == 'H':
                    nbH += 1
            if len(linker.molecule.atoms) - nbH > 1:
                # creation d'une instance de Brique -> ajoute a linkApp
                linkApp.append(Brique(linker.molecule, []))
            toIsolate = []
            linker = copy.deepcopy(brique)
    ...
    def graph(self, verbose=0):
        ...
        for i in range(len(linkApp)):
            ...
            for atom in linkApp[i].molecule.atoms:  # pr chaque atome de cette brique
                if atom.rings:  # c'est par cet atome que le linker/appendice est lie a un cycle
                    # on ajoute ds un dictionnaire l'index de cet atom que l'on retrouve grace a son num unique 'handle'
                    num[atom.handle] = atom.index
            if verbose:
            ...
            for j in range(len(cycles)):
                for atom in cycles[j].molecule.atoms:
                    if atom.handle in num.keys():
                        ...
                        if verbose:
                            sys.stderr.write("linker %d (%s) cycle %d (%s) linker atom is %d %s %d %d\n" % (i, linkApp[i].molecule.cansmiles(
                            ), j, cycles[j].molecule.cansmiles(), atom.index, atom.symbol, atom.handle, num[atom.handle]))
                        # (Num cycle, atom index in cycle, atom index in linker)
                        appToCycles[i].append(
                            (j, atom.index, num[atom.handle]))
                        traites += 1
                        break
                if traites == len(num):
                    break
        ...
        for app_cycle in appToCycles[0]:
            ...
            graph.append([cycles[app_cycle[0]], app_cycle[1],
                          linkApp[0], app_cycle[2]])
            ...
            while i < len(appToCycles):
                ...
                for j in range(len(appToCycles[i])):
                    ...
                    if appToCycles[i][j][0] in cyclesVisites:
                        graph.append(
                            [cycles[appToCycles[i][j][0]], appToCycles[i][j][1], linkApp[i], appToCycles[i][j][2]])
                        if verbose:
                            sys.stderr.write("Cycle %s atom %s %d linked to linker %d atom %s %d\n" % (cycles[appToCycles[i][j][0]].cansmiles(), cycles[appToCycles[i][j][0]].molecule.atoms[appToCycles[i][j][1]].symbol, cycles[
                                             appToCycles[i][j][0]].molecule.atoms[appToCycles[i][j][1]].index, i, linkApp[i].molecule.atoms[appToCycles[i][j][2]].symbol, linkApp[i].molecule.atoms[appToCycles[i][j][2]].index))
                ...
                if not done:
                    ...
                    while len(appToCycles[i]) > 0:
                        ...
                        graph.append(
                            [cycles[appToCycles[i][0][0]], appToCycles[i][0][1], linkApp[i], appToCycles[i][0][2]])
                        if len(appToCycles[i]) == 1:
                            del(appToCycles[i][0])
                            break
                        else:
        ...
        if verbose:
            for cnx in graph:
                sys.stderr.write("%s - %s-%d linked to %s - %s-%d\n" % (cnx[0].molecule.cansmiles(), cnx[0].molecule.atoms[cnx[1]].symbol,
                                                                        cnx[0].molecule.atoms[cnx[1]].index, cnx[2].molecule.cansmiles(),  cnx[2].molecule.atoms[cnx[3]].symbol, cnx[2].molecule.atoms[cnx[3]].index))
            # sys.exit(0)
        return graph
    ...
    def graph2molecule(self, workingPath, verbose=0):
        ...
        if (len(data[0]) == 1) and (not data[0][0].molecule.cycles):
            ...
            if len(data[0][0].molecule.atoms) >= 3:
                ...
                if len(data[0][0].molecule.atoms) == 2:
                    ...
                    data[0][0].molecule.atoms[1].x = dist(
                        data[0][0].molecule.atoms[0], data[0][0].molecule.atoms[1])
                return

            data[0][0].molecule.atoms[i].x = data[0][0].molecule.atoms[i].y = data[0][0].molecule.atoms[i].z = 1.0

            data[0][0].molecule.atoms[j].x = 1.0 + \
                dist(data[0][0].molecule.atoms[i],
                     data[0][0].molecule.atoms[j])
            data[0][0].molecule.atoms[j].y = data[0][0].molecule.atoms[j].z = 1.0

            dist_ik = dist(data[0][0].molecule.atoms[i],
                           data[0][0].molecule.atoms[k])
            angle_jik = angle(data[0][0].molecule.atoms[j], data[0][0].molecule.atoms[i],
                              data[0][0].molecule.atoms[k])
            data[0][0].molecule.atoms[k].x = 1.0 + \
                math.cos(angle_jik) * dist_ik
            data[0][0].molecule.atoms[k].y = 1.0 + \
                math.sin(angle_jik) * dist_ik
            data[0][0].molecule.atoms[k].z = 1.0

            if len(data[0][0].molecule.atoms) > 3:
                for index in [i, j, k]:
                    setLinkAppCoord(
                        0, data, data[0][0].molecule.atoms[index], self, saveindex=self.saveindex, verbose=verbose)

            for atom in data[0][0].molecule.atoms:
                at = self.molecule.atoms[getIndexAtom(
                    getAtom(atom.handle, self.molecule), self.molecule)]
                at.x = atom.x
                at.y = atom.y
                at.z = atom.z
        ...
        for i in range(len(data)):
            ...
            if data[i][0].molecule.atoms[0].x == None:  # mapping du cycle si c pas encore fait
                if verbose:
                    sys.stderr.write(
                        "graph2molecule: will manage a ring ...\n")
                try:
                    if verbose:
                        sys.stderr.write(
                            "graph2molecule: will map cycle %s\n" % workingPath)
            ...
            for bond in data[i][2].molecule.bonds:  # pr chaque liaison du linker
                if verbose:
                    sys.stderr.write(
                        "graph2molecule: will manage an appendix for %s ...\n" % data[i][2].molecule.cansmiles())

                for la in bond.atoms:  # pr chaque atome de la liaison du linker
                    # si c'est l'atome qui lie la brique a un cycle
                    if la == data[i][2].molecule.atoms[data[i][3]]:
                        ...
                        data[i][0].molecule.atoms[data[i][1]
                                                  ].oatoms.append(bond.xatom(la))
                        data[i][0].molecule.atoms[data[i]
                                                  [1]].bonds.append(bond)
            ...
            for at in data[i][2].molecule.atoms:
                if at.x == None:
                    ...
                    break
            # if data[i][2].molecule.atoms[data[i][3]].x != None :  bouger coordonnees du cycle pr le mettre au bon endroit si le linker est deja construit
            if linkAppCreated:
                # coordonnees sur lesquelles on va superposer les coordonnees du cycle a deplacer, pr creer la matrice de transformation
                if verbose:
                    sys.stderr.write(
                        "graph2molecule: Appendix coordinates are there ...\n")
                ...
                ori = self.molecule.atoms[getIndexAtom(
                    data[i][2].molecule.atoms[data[i][3]], self.molecule)]
                    ...
                    for abond in data[i][2].molecule.atoms[data[i][3]].bonds:
                        ...
                        if abond.xatom(data[i][2].molecule.atoms[data[i][3]]) == l[0]:
                            if abond.dbo:
                                ...
                                if sens == 2:
                                    sens = -1
                crds = setCoord(data[i][2].molecule.atoms[data[i][3]], l, r,
                                nbOfGhostsToPlace, sens, saveindex=self.saveindex, verbose=verbose)
                ...
                if self.axialEquatorial:
                    ...
                    if verbose:
                        sys.stderr.write(
                            "graph2molecule: axialEquatorial consideration\n")

                    toCheck = False
                    crdsL = []
                    for atom in l:
                        if atom.symbol != 'H':
                            descript = self.axialEquatorial[getIndexAtom(
                                atom, self.molecule)]
                            if descript in ['A', 'E']:
                    ...
                    if (len(crdsL) == 2) and (toCheck) and not (isAxEqValidPosition(data[i][2].molecule.atoms[data[i][3]], data[i][0].molecule.atoms, crdsL, descript)):
                        ...
                        bf.bestFit()
                        # deplace de cycle a la bonne place grace a la matrice de transformation
                        TM2(data[i][0].molecule.atoms,
                            bf.M1, bf.M2, bf.M3, bf.M4)
                ...
                for bond in data[i][0].molecule.atoms[data[i][1]].bonds:
                    if (bond.dbo) and (bond not in data[i][0].molecule.bonds):
                        ...
                        v34 = [crds[index4][0] - ori2.x, crds[index4]
                               [1] - ori2.y, crds[index4][2] - ori2.z]
                        scalar = v12[0] * v34[0] + \
                            v12[1] * v34[1] + v12[2] * v34[2]
            ...
            # if the linker/appendice has not yet been created
                ...
                setLinkAppCoord(i, data, ori, self,
                                saveindex=self.saveindex, verbose=verbose)
                data[i][2].molecule.atoms[data[i][3]
                                          ].x = data[i][0].molecule.atoms[data[i][1]].x
                data[i][2].molecule.atoms[data[i][3]
                                          ].y = data[i][0].molecule.atoms[data[i][1]].y
                data[i][2].molecule.atoms[data[i][3]
                                          ].z = data[i][0].molecule.atoms[data[i][1]].z
        ...
        if verbose:
            sys.stderr.write(
                "graph2molecule: will manage ring hydrogens ...\n")
        ...
        for cycle in self.molecule.cycles:
            for atom in cycle.atoms:
                ...
                for oatom in atom.oatoms:
                    ...
                    crds = setCoord(
                        atom, v, nb, saveindex=self.saveindex, verbose=verbose)
                    if verbose:
        ...
        if verbose:
            sys.stderr.write(
                "graph2molecule: installing conformer default dihedral values...\n")
        self.defaultDihedrals()
        self.graphCreated = True
        if verbose:
    ...
    def writeToSdf(self, file="mol", fournisseur=""):
        ...
        result.append("%3i%3i  0  0  0  0  0  0  0  0  1" %
                      (numatoms, numbonds))
        # ATOME BLOC
        for atom in self.molecule.atoms:
            x = atom.x
            ...
            if not z:
                z = 0.
            result.append("%10.4f%10.4f%10.4f%3s%3i%3i%3i%3i%3i" %
                          (x, y, z, atom.symbol, 0, 0, 0, 0, 0))
        # TABLE DE CONNECTIVITE
        for bond in self.molecule.bonds:
            result.append("%3i%3i%3i%3i%3i%3i" % (
    ...
    def writeToPdb(self, file="mol", withH=True):
        ...
        for atom in self.molecule.atoms:
            ...
            if not z:
                z = 0.
            result.append("ATOM" + str(atom.index + 1).rjust(7) + "  " + atom.symbol.upper().ljust(2) +
                          "  UNK     0" + "%12.4f%8.4f%8.4f" % (x, y, z) + "  1.00  0.00" + atom.symbol.upper().rjust(12))
        # TABLE DE CONNECTIVITE
        for atom in self.molecule.atoms:
            if (not withH) and (atom.symbol == 'H'):
                continue
        ...
        for bond in self.molecule.bonds:
            ...
            for fr in fRings:
                if (bond.atoms[0] in fr) and (bond.atoms[1] in fr):
                    if verbose:
                        sys.stderr.write(
                            "getDiedres: flush since in same ring\n")
                    inRing = 1
                    break
            ...
            if (not inRing) and (bond.bondtype == 1) and (bond not in toExclude):
                ...
                if (len(bond.atoms[0].oatoms) == 1) or (len(bond.atoms[1].oatoms) == 1):
                    if verbose:
                        sys.stderr.write(
                            "getDiedres: only one neighbour for one atom\n")
                    continue
                ...
                for i in range(2):
                    ...
                    for atom in bond.atoms:
                        ...
                        atomsAfterBond = getDividedMoleculeAtoms(
                            atoms[1].findbond(atoms[2]))[1]
                        if verbose:
    ...
    def defaultDihedrals(self, setAngle=True, verbose=0):
        ...
        for d in self.diedres:
            ...
            vals = getDihedralAngleValues(
                d.atoms[0], d.atoms[1], d.atoms[2], d.atoms[3])
            if verbose:
            ...
            if setAngle:
                ...
                angleRot = (
                    self.defaultDihedralValues[-1][0][0] * math.pi) / 180 - d.value
                if verbose:
    ...
    def to1confWWW(self, fileName, format="smi ", refFileName=None, energeticBarrer=100, eIni=300, nbBestResults=20, mcsteps=100, mode="mono", minimize=False, ammp_energy=False, autoMinimize=False, clusterize=False, clus_trhld=0.8, visuSdfFileName="/dev/null/", split=False, cleanup=0, vibrate=0, miniEach=False, verbose=0):
        """
        ...
        if not self.graphCreated:
            if verbose:
                sys.stderr.write(
                    "to1confWWW: Will generate 3D conformation ... \n")
            if fileName.find("/") != -1:
                self.graph2molecule(
                    fileName[0:fileName.rfind("/") + 1], verbose=verbose)
            else:
        ...
        if not self.diedres:
            ...
            coordsToMol2(self, [coords], ["no computed"],
                         "%s.ori.mol2" % filePrefix)
            # PT 13/01/2011
            if BABEL_VERSION < 220:
                babel_reprotonate_mol2_old(
                    filePrefix, ".ori.mol2", ".mol2", verbose=verbose)
            else:
                babel_reprotonate_mol2(
                    filePrefix, ".ori.mol2", ".mol2", verbose=verbose)

            # Now we need to convert into proper format
            if format == "mol2":
                if not split:
                    cmd = "cat %s.mol2 >> %s.mol2" % (
                        filePrefix, fileNamePrefix)
                    os.system(cmd)
            elif format == "pdbqt":
                babel_convert_split("%s.mol2" % filePrefix, "mol2", fileNamePrefix + "%s.pdbqt" %
                              filePrefix, "pdbqt", verbose=verbose)
            elif format == "pdb":
                babel_convert("%s.mol2" % filePrefix, "mol2",
                              "%s.pdb" % filePrefix, "pdb", verbose=verbose)
                if not split:
                ...
                babel_convert("%s.mol2" % filePrefix, "mol2",
                              "%s.sdf" % filePrefix, "sdf", verbose=verbose)
                if not split:
        ...
        # babel_convert("%s.mol2" % fileNamePrefix, "mol2", "%s" % visuSdfFileName, "sdf", verbose = verbose)
            ...
            babel_convert("%s.mol2" % filePrefix, "mol2", "%s.sdf" %
                          filePrefix, "sdf", verbose=verbose)
        ...
        for atom in self.molecule.atoms:
            ...
            tmp += str(atom.x) + " " + str(atom.y) + " " + str(atom.z) + " " + str(getMMFFPartialAtomicCharge(atom)) + " " + \
                str(vdwParams[0]) + " " + str(vdwParams[1]) + " " + \
                str(vdwParams[2]) + " " + str(vdwParams[3]) + "\n"
        # """
        ...
        for index, diedre in enumerate(self.diedres):
            tmpp += str(diedre.atoms[0].index) + " " + str(diedre.atoms[1].index) + \
                " " + str(diedre.atoms[2].index) + \
                " " + str(diedre.atoms[3].index)
            if verbose:
                ...
                sys.stderr.write("diedre %d %s-%d-%d %s-%d-%d %s-%d-%d %s-%d-%d peplike: %d\n" % (index, diedre.atoms[0].symbol, diedre.atoms[0].index, diedre.atoms[0].mmffAtomType, diedre.atoms[1].symbol, diedre.atoms[
                                 1].index, diedre.atoms[1].mmffAtomType, diedre.atoms[2].symbol, diedre.atoms[2].mmffAtomType, diedre.atoms[2].index, diedre.atoms[3].symbol, diedre.atoms[3].mmffAtomType, diedre.atoms[3].index, pepLikes[index]))
                sys.stderr.write("%s\n" % str(
                    (diedre.atoms[2].rings == []) and (diedre.atoms[3].rings == [])))
            ...
            if pepLikes[index] == 1:
                ...
                for j in range(len(self.defaultDihedralValues[index])):
                    tmpp += " " + str(self.defaultDihedralValues[index][j][0]) + " " + str(
                        self.defaultDihedralValues[index][j][1])
                tmpp += "\n"
        ...
        coordsToMol2(self, [coords], ["no energy computed"],
                     filePrefix + ".ori.mol2")
        if verbose:
        ...
        if refFileName == None:
            ...
            if BABEL_VERSION < 220:
                ...
                babel_reprotonate_mol2(
                    filePrefix, ".ori.mol2", ".babel.mol2", verbose=verbose)
        else:
...
#				fi.write(tmp)
        ...
        cmd = "%s %s -idata %s.ori.data -imol2 %s.babel.mol2 -omol2 %s.mol2 -id %s -eIni %f" % (
            MONOCONFv2, genMode, filePrefix, filePrefix, filePrefix, self.id, eIni)
        if vibrate:
            cmd = "%s -vb " % cmd
        ...
        # 	sys.stderr.write("%s %s\n" % (self.id, lines[0]))
        # Will just return the number of confs generated.
        lines = p.communicate(tmp)[0]
        ...
        if clusterize:
            cmd = "%s -imol2 %s.mol2 -omol2 %s-cluster.mol2 -rmsd %f" % (
                CLUSTCMD, filePrefix, filePrefix, clus_trhld)
            if verbose:
                sys.stderr.write("%s\n" % cmd)
            os.system(cmd)
        ...
        if verbose:
            sys.stderr.write(
                "Will check Energy is correct: %lf / %lf\n" % (float(bestEnergy), eIni))
        if format == "mol2":
            if not split:
                if float(bestEnergy) > eIni:
                    cmd = "cat %s.mol2 >> %s.mol2" % (
                        filePrefix, badEnergyFileNamePrefix)
                else:
                    cmd = "cat %s.mol2 >> %s.mol2" % (
                        filePrefix, fileNamePrefix)
                os.system(cmd)
        elif format == "pdb":
            babel_convert("%s.mol2" % filePrefix, "mol2", "%s.pdb" %
                          filePrefix, "pdb", verbose=verbose)
            if not split:
                if float(bestEnergy) > eIni:
                    cmd = "cat %s.pdb >> %s.pdb" % (
                        filePrefix, badEnergyFileNamePrefix)
        ...
        if nbCycles == 1 and len(atom.rings[indexCycle].atoms) > 4:
            ...
            if len(neighbours) == 2:
                ...
                for oatom in neighbours:
                    chainsNeighbours.append(
                        toCanonicalChain(oatom, oatom.findbond(atom)))
    ...
    def toNsmiles_new(self, axialEquatorialPos=False, verbose=0):
        ...
        if verbose:
            sys.stderr.write(
                "toNsmiles: Will identify @ and axial/equatorial sites\n")
        ...
        while indexBond < (len(self.molecule.bonds) - len(self.molecule.atoms) + self.numberOfNonHAtoms):
            ...
            if (not isChiral) and ((not self.molecule.bonds[indexBond].dbo) and (isChiralBond(self.molecule.bonds[indexBond]))):
                ...
                if atoms[0].index != 0:
                            ...
                            if indexAtom == atoms[0].index:
                                ...
                                while True:
                                    ...
                                    if i >= len(self.smiles):
                                        sys.stderr.write(
                                            "toNsmiles: overflow on smiles walk\n")
                                        raise Exception(
                                            "toNsmiles: overflow on smiles walk")
                                    if self.smiles[i] == '[':
                                        openBracket = True
                    ...
                    if verbose:
                        sys.stderr.write(
                            "Found special init atom at smiles %d\n" % init)
                    stereoIsos.append(("ZE1", init))
                    indexAtom = 0
                    i = 0
                    atom = self.molecule.atoms[indexAtom]
                    ...
                    while i < len(self.smiles):
                        if self.smiles[i] == '[':
                            ...
                            if (envers) and (indexAtom == nextIndex):
                                ...
                                if verbose:
                                    sys.stderr.write(
                                        "Found special end atom at smiles %d\n" % i)
                                break
        ...
        if verbose:
            sys.stderr.write(
                "toNsmiles: found %d @/@@ and ZE sites\n" % (len(stereoIsos)))
        ...
        for i in range(lg - 1):
            if stereoIsos[i - deleted][1] == stereoIsos[i + 1 - deleted][1]:
                #stereoIsos[i][0] = "ZE2"
                stereoIsos[i + 1 -
                           deleted] = ("ZEX", stereoIsos[i + 1 - deleted][1])
        ...
        if verbose:
            sys.stderr.write(
                "toNsmiles: will generate unambiguous smiles for %d enantiomer sites\n" % len(stereoIsos))
        ...
        if stereoIsos:
            ...
            if verbose:
                sys.stderr.write(
                    "toNsmiles: will generate %d unambiguous smiles\n" % len(nSmiles))
            ...
            while i < len(self.smiles):
                ...
                for j in xrange(len(nSmiles)):
                    nSmiles[j] = nSmiles[j] + \
                        self.smiles[i:stereoIsos[indexStereo][1]]
                    ...
                    nSmiles[j] = nSmiles[j] + \
                        stereos[stereoIsos[indexStereo][0]][whichStereo]
