...
def checkSDF(fname):
    import tempfile
    dummy, iFile3DSdf = tempfile.mkstemp(".smi", "tmpFrog", "./")
...
def usage():
    sys.stderr.write(
        "www_iMolecule.py <options> <-ismi,-isdf,-imol2> inputfile\n")
    sys.stderr.write("\nDefaults: -mono -wrkPath . -unambiguate -automini -logFile /dev/stdout\n\n")
    ...
    sys.stderr.write(
        " -i3Dsdf file: 3D SDF input file (no generation from scratch, just multiconf)\n")
    sys.stderr.write(
        " -i3Dmol2 file: 3D mol2 input file (no generation from scratch, just multiconf)\n")
    sys.stderr.write(" -opdb file: PDB ouput file\n")
    sys.stderr.write(" -opdbqt file: PDBQT ouput file\n")
    ...
    sys.stderr.write(
        " -ounsolved file: file to list compounds not generated\n")
    sys.stderr.write(" -wrkPath file: working directory\n\n")

    sys.stderr.write(
        " -mono: generate only one conformer per stereo-isomer.\n")
    sys.stderr.write(" -multi x: generate x conformers per stereo-isomer.\n")
    sys.stderr.write(
        " -unambiguate: enable search for undefined stereo centers.\n")
    sys.stderr.write(" -vb: enable stage two Monte-Carlo.\n")
    sys.stderr.write(
        " -mcsteps x: # steps for stage two Monte-Carlo (limit to small values since costly. 100 is usually OK).\n")
    sys.stderr.write(
        " -emax x: energy slice relative to lowest energy compound. 50, 70 OK since approximate fast energy)\n")
    sys.stderr.write(
        " -eini x: energy value to declare compound of high energy (500 OK).\n")
    sys.stderr.write(
        " -automini: minimize compounds of high energy on exit (reasonably slow).\n")
    sys.stderr.write(" -mini: minimize compounds on exit (very slow).\n")
    sys.stderr.write(
        " -miniEach: minimize compounds during generation (very very slow!).\n")
    sys.stderr.write(
        " -gnb: output multi conf over all isomers (and not for each isomer).\n")
    sys.stderr.write(
        " -rmsd x: output only compounds different by more than x Angstroms RMSd.\n")
    sys.stderr.write(
        " -maxMols x: limit gereation to maxMols first compounds.\n\n")
    sys.stderr.write(" -split: each compound in a different file.\n\n")

    sys.stderr.write(
        " -pymol: generate pymol script and produce compound image.\n")
    ...
    try:
        while i < len(sys.argv):
            ...
            if (sys.argv[i] == "-ismi") and (i + 1 < len(sys.argv)):
                ...
                o3dFileFormat = "pdb"
                i += 2
            elif (sys.argv[i] == "-opdbqt") and (i + 1 < len(sys.argv)):
                o3dFileName = sys.argv[i + 1]
                split = True
                # o3dFileName = 'dummy'
                o3dFileFormat = "pdbqt"
...
# we maintain mols (compound SMILES) and ids (id from file or index if not)
            with smart_open(iFileSmiles) as fh:
                fSmiles = fh
                lines = fSmiles.readlines()
                fSmiles.close()
...
# $$$$ is the delimiter to finish compound
            ...
            if not mols:
                raise Exception(
                    "some problem occurred while readin the sdf (check its format)")
            sdfTmpFile = tempfile.NamedTemporaryFile(
                suffix='.sdf', prefix="Frog2-", dir=wrkPathTmp)
        ...
        if logFile:
            ...
            if multi:
                logF.write(",  nb max conformations   : %5s" %
                           str(nbBestResults))
        ...
        if verbose:
            sys.stderr.write(
                "Input of parameters and data done ... Will manage %d mol(s)\n" % len(mols))
        ...
        for i in range(len(mols)):
            try:
                ...
                if i == maxMols and logFile:
                    logF.write(
                        "maximum authorized number of molecules processed reached\n")
                    break
                id = ids[i]
                if not id:
                    id = "mol_" + str(i + 1)
                ...
                if iFileSmiles and nStereos:
                    ...
                    if verbose:
                        sys.stderr.write(
                            "Successfully loaded sdf (cansmiles %s)\n" % theMol.cansmiles())
                    iMol = iMolecule(smiles=theMol.cansmiles(),
                                     id=id, verbose=verbose)
                    if logFile and iMol.bigRingWarning:
                        logF.write(
                            "   [WARNING] Detected large rings that could be managed poorly by Frog (see limitations of Frog for bridged rings).\n")
                    if verbose:
                    ...
                    iMol = iMolecule(sdfMolecule=frowns_sdf,
                                     id=id, extractCoordsFromSdf=True)
                    if verbose:
                    ...
                    if multi:
                        if verbose:
                            sys.stderr.write(
                                "Will generate multiconf from 3D SDF\n")
                        nbC += iMol.to1confWWW(fileName=o3dFileName, mode="multi", minimize=minimize, autoMinimize=autoMinimize, ammp_energy=ammp_energy, clusterize=clusterize, clus_trhld=clus_trhld, format=o3dFileFormat, nbBestResults=nbBestResults,
                                               energeticBarrer=eMax, eIni=eIni, mcsteps=mcSteps, visuSdfFileName=wrkPathTmp + "/mol-%d.sdf" % (i + 1), split=split, refFileName=iRefFileName, vibrate=vibrate, miniEach=miniEach, verbose=verbose)
                        if doPymol:
                            if mode == "mono":
                                genPmlScript("mol-%d.sdf" % (i + 1))
                            else:
                                genPmlScript("mol-%d.sdf" %
                                             (i + 1), allStates=True)
                            doSnap()
                        if logFile:
                            if iMol.eOK:
                                logF.write(
                                    "   %s conformation(s) calculated.\n\n" % (str(nbC)))
                                logF.flush()
                            else:
                                logF.write(
                                    "   %s conformation(s) calculated but have bad energies.\n\n" % (str(nbC)))
                                logF.flush()
                    else:
                        logF.write(
                            "   no new conformation calculated from 3D sdf\n\n")
                        logF.flush()
                    continue
                if nStereos:
...
# and we also identify axial / equatorial sites
                    ...
                    smiles, axsEqs, removed = iMol.toNsmiles(
                    ...
                    if logFile:
                        if n > 8:
                            ...
                            logF.write(
                                "   " + str(n) + " isomer(s) found (see the smiles file)\n")
                            logF.write(
                                "   [WARNING] number of isomers considered restrainted to %d (random selection is performed):\n" % rndSmiles)
                            if removed:
                                logF.write(
                                    "   [WARNING] the chirality of %d stereo centers specified in the input has been withdrawn (see limitations of Frog for bridged rings).\n" % removed)
                            axsEqs = [None]
                            ...
                            if removed:
                                logF.write(
                                    "   [WARNING] the chirality of %d stereo center(s) specified in the input has been withdrawn (see limitations of Frog for bridged rings).\n" % removed)
                            logF.flush()
                else:
...
# We do not disambiguate.
                ...
                if verbose:
                    sys.stderr.write(
                        "Will perform 3D generation ... sampleStereo is %d\n" % int(sampleStereo))
...
# Now we gererate 3D conformations for each isomer.
                ...
                for smile in smiles:
                    for axEq in axsEqs:
                        iMol = iMolecule(smiles=smile,  id=id + "_" + str(
                            j), axialEquatorial=axEq, sampleStereo=sampleStereo, verbose=verbose)
...
# We always generate 1 conformation, and additional file
                        ...
                        if verbose:
                            sys.stderr.write(
                                "Will perform 3D conformer generation for %s...\n" % id)
                        mode = "mono"
                        if multi:
                            mode = "multi"
                        lnbC = iMol.to1confWWW(fileName=o3dFileName, mode=mode, minimize=minimize, autoMinimize=autoMinimize, ammp_energy=ammp_energy, clusterize=clusterize, clus_trhld=clus_trhld, format=o3dFileFormat, nbBestResults=lnbBestResults,
                                               energeticBarrer=eMax, eIni=eIni, mcsteps=mcSteps, visuSdfFileName=wrkPathTmp + "/mol-%d.sdf" % (i + 1), split=split, refFileName=iRefFileName, vibrate=vibrate, miniEach=miniEach, verbose=verbose)
                        if doPymol:
                            if mode == "mono":
                                genPmlScript("mol-%d.sdf" % (i + 1))
                ...
                if multi and logFile:
                    ...
                    if nStereos:
                        logF.write(
                            "   1 conformation(s) calculated for each isomer. %d have bad energies.\n\n" % highEnbC)
                        logF.flush()
                    else:
                ...
                if logFile:
                    logF.write(
                        "  [ERROR] some energy parameters may be missing (is the molecule ADME/tox compliant?)\n")
                    logF.flush()
                if unsolvedFileName:
                ...
                logF.write(
                    "  [ERROR] an internal problem occured. Please try again this compound\n")
                logF.flush()
            except Exception, err:
                print err
                if logFile:
